...
From IC.if_convergent.derived Require Import IC_indexed_step_fupd.
...
Section ic_IC_ISF.
  ...
  Definition icd_IC_ISF : ICData Λ Σ :=
    {| ICD_state_interp := ICD_SI;
      ...
      ICD_modality idx E n Φ :=
        IC@{icd_indexed_step_fupd ICD_SI', n }
          idx @ E {{ w; k, Φ (w, k) }}%I;
    |}.

  Global Instance ICC_IC_ISF : ICC icd_IC_ISF.
  Proof.
    split.
    ...
    - iIntros (idx E n Φ) "H";simpl.
      iApply ic_indexed_step_fupd_index_intro; last eauto. simpl; lia.
    - intros e e' f g E n m Φ (K & HK & He & Hg & Hf); simplify_eq.
      iIntros "H"; simpl.
      iApply ic_indexed_step_fupd_bind; simpl.
  ...
  Global Instance ICC_IC_ISF_is_outer_fupd idx :
    ICMIsOuterModal icd_IC_ISF idx (λ E _ P, |={E}=> P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  Qed.

  Global Instance ICC_IC_ISF_is_outer_bupd idx :
    ICMIsOuterModal icd_IC_ISF idx (λ _ _ P, |==> P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  Qed.

  Global Instance ICC_IC_ISF_is_outer_except_0 idx :
    ICMIsOuterModal icd_IC_ISF idx (λ _ _ P, ◇ P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  ...
  Global Instance ICC_IC_ISF_is_inner_fupd idx :
    ICMIsInnerModal icd_IC_ISF idx (λ E _ P, |={E}=> P)%I.
  Proof.
  ...
  Global Instance ICC_IC_ISF_is_inner_bupd idx :
    ICMIsInnerModal icd_IC_ISF idx (λ _ _ P, |==> P)%I.
  Proof.
    rewrite /ICMIsInnerModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H".
  ...
  Global Instance ICC_IC_ISF_is_inner_except_0 idx :
    ICMIsInnerModal icd_IC_ISF idx (λ _ _ P, ◇ P)%I.
  Proof.
    rewrite /ICMIsInnerModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H".
  ...
  Global Instance ICC_IC_ISF_SupportsAtomicShift idx :
    ICMSupportsAtomicShift icd_IC_ISF idx.
  Proof.
  ...
  Global Program Instance ICC_IC_ISF_SplitForStep idx :
    ICMSplitForStep icd_IC_ISF idx :=
    {|
  ...
  Lemma ic_IC_ISF_strong_bind
        K `{!LanguageCtx K} K' `{!LanguageCtx K'} E e e' Φ :
    IC@{icd_IC_ISF, e'} e @ E {{ v; m | w ; k,
      IC@{icd_IC_ISF, K' (of_val w)}
        K (of_val v) @ E {{ w; n | u ; y, Φ w (m + n) (u, (k + y)) }} }}
    ⊢ IC@{icd_IC_ISF, K' e'} K e @ E {{ Φ }}.
  Proof.
    iIntros "H".
    iApply (@ic_bind _ _ _ ICC_IC_ISF K _ _ e'
                     (λ '(v, k), K' (of_val v))
                     (λ x y, (y.1, x.2 + y.2))).
...
Lemma ic_IC_ISF_change_of_index e1' e2' f E e Φ :
  (∀ Ψ n,
      IC@{icd_indexed_step_fupd ICD_SI', n}
        e1' @ E {{ v; n | [_], Ψ (f (v, n)) }} -∗
        IC@{icd_indexed_step_fupd ICD_SI', n}
        e2' @ E {{ v; n | [_], Ψ (v, n) }})
  -∗ IC@{icd_IC_ISF, e1'} e @ E {{ λ v n x, Φ v n (f x) }}
  -∗ IC@{icd_IC_ISF, e2'} e @ E {{ Φ }}.
Proof.
  iIntros "Hm H".
  iApply (ic_change_of_index icd_IC_ISF with "[Hm] H").
  iIntros (Ψ n) "H".
  by iApply "Hm".
Qed.
    ...
    (∀ σ1, ICD_SI σ1 -∗
     ...
     ▷ ∀ e2 σ2,
         ⌜prim_step e1 σ1 [] e2 σ2 []⌝ ={∅, E}=∗
            (ICD_SI σ2 ∗ IC@{icd_IC_ISF ICD_SI ICD_SI', idx}
                    e2 @ E {{ v; n| [x], Φ v (S n) x }}))
    ⊢ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros (?) "?".
  iApply (ic_lift_step (icd_IC_ISF ICD_SI ICD_SI') idx E); simpl; auto.
Qed.

Lemma ic_IC_ISF_lift_pure_step idx E Φ e1 :
  to_val e1 = None →
  (∀ σ1 e2 σ2, prim_step e1 σ1 [] e2 σ2 [] → σ1 = σ2) →
  ▷ (∀ σ1 e2 σ2,
      ⌜prim_step e1 σ1 [] e2 σ2 []⌝ →
      IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e2 @ E
        {{ v; n | [x], Φ v (S n) x }})
    ⊢ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros(??) "H".
  iApply (ic_lift_pure_step
            (icd_IC_ISF ICD_SI ICD_SI') idx E);
    simpl; eauto.
  by iApply step_fupd_intro; first set_solver.
Qed.

Lemma ic_IC_ISF_lift_atomic_step idx E Φ e1 :
  to_val e1 = None →
   ...
   (∀ σ1, ICD_SI σ1 ={E, ∅}=∗
        ▷ ∀ v2 σ2,
            ⌜prim_step e1 σ1 [] (of_val v2) σ2 []⌝
             ={∅, E}=∗ (ICD_SI σ2 ∗
                        IC@{icd_indexed_step_fupd ICD_SI', 0}
...
Lemma ic_IC_ISF_lift_atomic_det_step idx E Φ e1 :
  to_val e1 = None →
  Atomic StronglyAtomic e1 →
  (∀ σ1, ICD_SI σ1 ={E, ∅}=∗
     ▷ (∃ v2 σ2,
...
Proof.
  by iIntros (??);
    iApply (ic_lift_atomic_det_step
              (icd_IC_ISF ICD_SI ICD_SI') idx E Φ e1).
Qed.

Lemma ic_IC_ISF_lift_pure_det_step idx E Φ e1 e2 :
  to_val e1 = None →
  (∀ σ1 e2' σ2, prim_step e1 σ1 [] e2' σ2 [] → σ1 = σ2 ∧ e2 = e2')→
  ▷ IC@{icd_IC_ISF ICD_SI ICD_SI', idx}
    e2 @ E {{ v; n | [x], Φ v (S n) x }}
  ⊢ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros(??) "H".
...
Lemma ic_IC_ISF_pure_step `{!Inhabited (state Λ)} idx E e1 e2 φ n Φ :
  PureExec φ n e1 e2 →
  φ →
  ▷^n IC@{icd_IC_ISF ICD_SI ICD_SI', idx}
    e2 @ E {{ v ; m | [x], Φ v (n + m) x }}
  ⊢ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros (Hexec Hφ) "Hic".
  iApply (ic_pure_step (icd_IC_ISF ICD_SI ICD_SI') idx); eauto.
  clear Hexec.
...
Lemma ic_IC_ISF_lift_head_step idx E Φ e1 :
  to_val e1 = None →
...
Lemma ic_IC_ISF_lift_pure_head_step idx E Φ e1 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
...
Lemma ic_IC_ISF_lift_atomic_head_step idx E Φ e1 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
  Atomic StronglyAtomic e1 →
   (∀ σ1, ICD_SI σ1 ={E, ∅}=∗
     ▷ ∀ v2 σ2,
       ⌜head_step e1 σ1 [] (of_val v2) σ2 []⌝ ={∅, E}=∗
          (ICD_SI σ2 ∗ IC@{icd_indexed_step_fupd ICD_SI', 0}
                          idx @ E {{ v; n, Φ v2 1 (v, n) }}))
     ⊢ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  by intros;
...
Lemma ic_IC_ISF_lift_pure_det_head_step idx E Φ e1 e2 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
...
Proof.
  iIntros (???) "?".
  iApply (ic_lift_pure_head_step'
            (icd_IC_ISF ICD_SI ICD_SI') idx E Φ e1); eauto.
   by iApply step_fupd_intro; first set_solver.
Qed.
...
Lemma ic_IC_ISF_adequacy_basic idx E e σ Φ :
  ICD_SI σ ∗ IC@{icd_IC_ISF ICD_SI ICD_SI', idx} e @ E {{ λ v n x, Φ v n x.1 x.2 }} -∗
    ∀ (rd : Reds e σ),
      IC@{icd_indexed_step_fupd ICD_SI', nstps rd}
                          idx @ E {{ v; n, ICD_SI (end_state rd) ∗
                                          Φ (end_val rd) (nstps rd) v n }}.
Proof.
  iApply (ic_adequacy_basic
            (icd_IC_ISF ICD_SI ICD_SI') idx E e σ (λ v n x, Φ v n x.1 x.2)).
Qed.
...
Program Instance IC_ISF_Initialization
        SSI SSI' `{!InitData SSI} `{!InitData SSI'} e' σ' :
  Initialization
    (val Λ * nat)
    (λ x : invG_pack Σ (SI_data * SI_data'),
           icd_IC_ISF (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2))
    (λ x, @ICC_IC_ISF Λ Σ _ (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2))
    (λ _, e') :=
{|
...
Proof.
  ...
  iNext.
  rewrite /ICC_modality /ICD_modality /=.
  iDestruct (ic_indexed_step_fupd_adequacy_basic with "[Hσ' $HP]")
    as "HP"; eauto.
  iSpecialize ("HP" $! rd).
  rewrite uPred_fupd_eq /uPred_fupd_def /=.
...
Lemma ic_IC_ISF_adequacy
  ...
  (∀ (x : invG_pack Σ (SI_data * SI_data')),
       ...
       |={⊤}=> (ICD_SI (PIG_cnt x).1 σ ∗ ICD_SI' (PIG_cnt x).2 σ' ∗
              IC@{icd_IC_ISF (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2), e'}
...
Proof.
  ...
  by apply (ic_adequacy
              _ _ _ _ (IC_ISF_Initialization SSI SSI' e' σ') E e σ
