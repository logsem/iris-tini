...
Section ic_logrel_fupd.
  ...
  Definition icd_logrel_fupd : ICData Λ Σ :=
    {| ICD_state_interp := ICD_SI;
      ...
      ICD_modality idx E n Φ :=
        IC@{icd_fupd ICD_SI'} idx @ E {{ w; k, Φ (w, k) }}%I;
    |}.

  Global Instance ICC_logrel_fupd : ICC icd_logrel_fupd.
  Proof.
    split.
  ...
  Global Instance ICC_logrel_fupd_is_outer_fupd idx :
    ICMIsOuterModal icd_logrel_fupd idx (λ E _ P, |={E}=> P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  Qed.

  Global Instance ICC_logrel_fupd_is_outer_bupd idx :
    ICMIsOuterModal icd_logrel_fupd idx (λ _ _ P, |==> P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  Qed.

  Global Instance ICC_logrel_fupd_is_outer_except_0 idx :
    ICMIsOuterModal icd_logrel_fupd idx (λ _ _ P, ◇ P)%I.
  Proof.
    rewrite /ICMIsOuterModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H". by iMod "H".
  ...
  Global Instance ICC_logrel_fupd_is_inner_fupd idx :
    ICMIsInnerModal icd_logrel_fupd idx (λ E _ P, |={E}=> P)%I.
  Proof.
  ...
  Global Instance ICC_logrel_fupd_is_inner_bupd idx :
    ICMIsInnerModal icd_logrel_fupd idx (λ _ _ P, |==> P)%I.
  Proof.
    rewrite /ICMIsInnerModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H".
  ...
  Global Instance ICC_logrel_fupd_is_inner_except_0 idx :
    ICMIsInnerModal icd_logrel_fupd idx (λ _ _ P, ◇ P)%I.
  Proof.
    rewrite /ICMIsInnerModal /ICC_modality /ICD_modality /=.
    iIntros (E n P) "H".
  ...
  (* Global Instance ICC_logrel_fupd_SupportsAtomicShift idx : *)
  (*   ICMSupportsAtomicShift icd_logrel_fupd idx. *)
  (* Proof. *)
  ...
  Global Program Instance ICC_logrel_fupd_SplitForStep idx :
    ICMSplitForStep icd_logrel_fupd idx :=
    {|
  ...
  Lemma ic_logrel_fupd_strong_bind
        K `{!LanguageCtx K} K' `{!LanguageCtx K'} E e e' Φ :
    IC@{icd_logrel_fupd, e'} e @ E {{ v; m | w ; k,
      IC@{icd_logrel_fupd, K' (of_val w)}
        K (of_val v) @ E {{ w; n | u ; y, Φ w (m + n) (u, (k + y)) }} }}
    ⊢ IC@{icd_logrel_fupd, K' e'} K e @ E {{ Φ }}.
  Proof.
    iIntros "H".
    iApply (@ic_bind _ _ _ ICC_logrel_fupd K _ _ e'
                     (λ '(v, k), K' (of_val v))
                     (λ x y, (y.1, x.2 + y.2))).
...
Lemma ic_logrel_fupd_change_of_index e1' e2' f E e Φ :
  (∀ Ψ n,
      IC@{icd_fupd ICD_SI', n}
        e1' @ E {{ v; n | [_], Ψ (f (v, n)) }} -∗
        IC@{icd_fupd ICD_SI', n}
        e2' @ E {{ v; n | [_], Ψ (v, n) }})
  -∗ IC@{icd_logrel_fupd, e1'} e @ E {{ λ v n x, Φ v n (f x) }}
  -∗ IC@{icd_logrel_fupd, e2'} e @ E {{ Φ }}.
Proof.
  iIntros "Hm H".
  iApply (ic_change_of_index icd_logrel_fupd with "[Hm] H").
  iIntros (Ψ n) "H".
  by iApply "Hm".
Qed.

Lemma ic_logrel_fupd_pure_step_index `{!Inhabited (state Λ)}  e1' e2' E e Φ φ n :
  PureExec φ n e2' e1' →
  φ →
  IC@{icd_logrel_fupd, e1'} e @ E {{ v;k | w;m, Φ v k (w, n + m) }}
  ⊢ IC@{icd_logrel_fupd, e2'} e @ E {{ v;m | [x], Φ v m x }}.
Proof.
  iIntros (Hexec Hφ) "Hic".
  iApply (ic_logrel_fupd_change_of_index _ _ (λ '(v, m), (v, n + m))).
  - iIntros (? []) "H".
    by iApply ic_fupd_pure_step.
    ...
    (∀ σ1, ICD_SI σ1 -∗
     ...
     ∀ e2 σ2,
         ⌜prim_step e1 σ1 [] e2 σ2 []⌝ ={∅, E}=∗
            (ICD_SI σ2 ∗ IC@{icd_logrel_fupd ICD_SI ICD_SI', idx}
                    e2 @ E {{ v; n| [x], Φ v (S n) x }}))
    ⊢ IC@{icd_logrel_fupd ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros (?) "?".
  iApply (ic_lift_step (icd_logrel_fupd ICD_SI ICD_SI') idx E); simpl; auto.
Qed.

Lemma ic_logrel_fupd_lift_pure_step idx E Φ e1 :
  to_val e1 = None →
  (∀ σ1 e2 σ2, prim_step e1 σ1 [] e2 σ2 [] → σ1 = σ2) →
   (∀ σ1 e2 σ2,
      ⌜prim_step e1 σ1 [] e2 σ2 []⌝ →
      IC@{icd_logrel_fupd ICD_SI ICD_SI', idx} e2 @ E
        {{ v; n | [x], Φ v (S n) x }})
    ⊢ IC@{icd_logrel_fupd ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros(??) "H".
  iApply (ic_lift_pure_step
            (icd_logrel_fupd ICD_SI ICD_SI') idx E);
    simpl; eauto.
  by iApply fupd_intro_mask; first set_solver.
Qed.

Lemma ic_logrel_fupd_lift_atomic_step idx E Φ e1 :
  to_val e1 = None →
...
Lemma ic_logrel_fupd_lift_atomic_det_step idx E Φ e1 :
  to_val e1 = None →
  Atomic StronglyAtomic e1 →
  (∀ σ1, ICD_SI σ1 ={E, ∅}=∗
     (∃ v2 σ2,
...
Proof.
  by iIntros (??);
    iApply (ic_lift_atomic_det_step
              (icd_logrel_fupd ICD_SI ICD_SI') idx E Φ e1).
Qed.

Lemma ic_logrel_fupd_lift_pure_det_step idx E Φ e1 e2 :
  to_val e1 = None →
  (∀ σ1 e2' σ2, prim_step e1 σ1 [] e2' σ2 [] → σ1 = σ2 ∧ e2 = e2')→
  IC@{icd_logrel_fupd ICD_SI ICD_SI', idx}
    e2 @ E {{ v; n | [x], Φ v (S n) x }}
  ⊢ IC@{icd_logrel_fupd ICD_SI ICD_SI', idx} e1 @ E {{ Φ }}.
Proof.
  iIntros(??) "H".
...
Lemma ic_logrel_fupd_pure_step `{!Inhabited (state Λ)} idx E e1 e2 φ n Φ :
  PureExec φ n e1 e2 →
  φ →
  IC@{icd_logrel_fupd ICD_SI ICD_SI', idx}
...
Proof.
  iIntros (Hexec Hφ) "Hic".
  iApply (ic_pure_step (icd_logrel_fupd ICD_SI ICD_SI') idx); eauto.
  clear Hexec.
...
Lemma ic_logrel_fupd_lift_head_step idx E Φ e1 :
  to_val e1 = None →
...
Lemma ic_logrel_fupd_lift_pure_head_step idx E Φ e1 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
...
Lemma ic_logrel_fupd_lift_atomic_head_step idx E Φ e1 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
  Atomic StronglyAtomic e1 →
   (∀ σ1, ICD_SI σ1 ={E, ∅}=∗
...
Lemma ic_logrel_fupd_lift_pure_det_head_step idx E Φ e1 e2 :
  to_val e1 = None →
  sub_redexes_are_values e1 →
...
Proof.
  iIntros (???) "?".
  iApply (ic_lift_pure_head_step'
            (icd_logrel_fupd ICD_SI ICD_SI') idx E Φ e1); eauto.
   by iApply fupd_intro_mask; first set_solver.
Qed.
...
Lemma ic_logrel_fupd_adequacy_basic idx E e σ Φ :
  ICD_SI σ ∗ IC@{icd_logrel_fupd ICD_SI ICD_SI', idx} e @ E {{ λ v n x, Φ v n x.1 x.2 }} -∗
    ∀ (rd : Reds e σ),
      IC@{icd_fupd ICD_SI'}
...
Proof.
  iApply (ic_adequacy_basic
            (icd_logrel_fupd ICD_SI ICD_SI') idx E e σ (λ v n x, Φ v n x.1 x.2)).
Qed.
...
Program Instance left_Initialization
  ...
  Initialization
    ...
    (λ x : invG_pack Σ (SI_data * SI_data'),
           icd_logrel_fupd (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2))
    (λ x, @ICC_logrel_fupd Λ Σ _ (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2))
    (λ _, e') :=
{|
  ...
  initialization_ICCM_soundness_laters n rd' := 2;
...
Proof.
  ...
  iDestruct (ic_fupd_adequacy_basic with "[Hσ' $HP]")
    as "HP"; eauto.
  iSpecialize ("HP" $! rd).
  rewrite uPred_fupd_eq /uPred_fupd_def /=.
...
Lemma ic_logrel_fupd_adequacy SSI SSI' `{!InitData SSI} `{!InitData SSI'}
  ...
  (∀ (x : invG_pack Σ (SI_data * SI_data')),
      SSI (PIG_cnt x).1 ∗ SSI' (PIG_cnt x).2
          ⊢ |={⊤}=> (ICD_SI (PIG_cnt x).1 σ ∗ ICD_SI' (PIG_cnt x).2 σ' ∗
                  IC@{icd_logrel_fupd (ICD_SI (PIG_cnt x).1) (ICD_SI' (PIG_cnt x).2), e'}
